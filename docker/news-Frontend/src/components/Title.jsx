...
function Title() {
  ...
  const news = location.state?.news; // Retrieve the data passed from Home.jsx


  if (!news) {
    return <p>No data available for this news item. Please go back to the homepage.</p>;
  }
  ...
  <div className="container mx-auto flex flex-col px-5 py-24 justify-center items-center">
    <img className="lg:w-2/6 md:w-3/6 w-5/6 mb-10 object-cover object-center rounded" alt="hero" src={news.image}/>
    <div className="w-full md:w-2/3 flex flex-col mb-16 items-center text-center">
      <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-white">{news.title}</h1>
      <p className="mb-8 leading-relaxed ">{news.summary}</p>
      ...
      <p className="text-sm mt-2 text-gray-500 mb-8 w-full">{news.fetched_at}</p>
